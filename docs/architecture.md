...
```text
байты → страницы → WAL → recovery → каталог → таблицы → транзакции → индексы → SQL → расширения
```
...
Абстракция файловой системы и синхронизации данных. Нужна для Linux, Windows, Android, тестов отказов и будущей fault injection. Ядро не должно напрямую зависеть от произвольных вызовов `std::fs` в бизнес-логике.
